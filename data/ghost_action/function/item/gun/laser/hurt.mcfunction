...
execute if function ghost_action:item/prop/furious/is at @n[tag=source] if entity @s[distance=8..35] run damage @s 4 mob_attack by @n[tag=source]
execute if function ghost_action:item/prop/furious/is at @n[tag=source] if entity @s[distance=36..100] run damage @s 4 mob_attack by @n[tag=source]
