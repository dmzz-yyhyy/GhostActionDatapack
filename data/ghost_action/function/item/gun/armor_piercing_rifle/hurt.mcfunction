...
execute unless function ghost_action:item/prop/furious/is at @n[tag=source] if entity @s[distance=..100] run damage @s 8 mob_attack by @n[tag=source]
execute if function ghost_action:item/prop/furious/is at @n[tag=source] if entity @s[distance=..100] run damage @s 7 mob_attack by @n[tag=source]
